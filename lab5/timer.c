...
const int timer_id = 10;	// arbitrary hook_id
...
int timer_subscribe_int(void) {
	...
	int hook_id = timer_id;	// local variable with the value of the hook id
	...
	return BIT(timer_id);	// returns a bit mask with BIT(hook_id) set to 1
}

int timer_unsubscribe_int() {

	int hook_id = timer_id;	// again a local variable, so the original remains unchanged
