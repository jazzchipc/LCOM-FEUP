...
int timer_subscribe_int(void) {
	...
	int hook_id = TIMER_HOOK_ID;	// local variable with the value of the hook id
	...
	return BIT(TIMER_HOOK_ID);	// returns a bit mask with BIT(hook_id) set to 1
}

int timer_unsubscribe_int() {

	int hook_id = TIMER_HOOK_ID;	// again a local variable, so the original remains unchanged
