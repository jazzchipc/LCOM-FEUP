...
/**
 * @brief Draws an unscaled, unrotated bitmap at the given (x,y) position, BUT IT IGNORES THE COLOR DEFINED IN THE PARAMETER!
 * The pixels of the bitmap with the color specified are not written to the video memory.
 * The value of "color" is calculated by the getIntFromRGB() function.
 * The list with the colors used is in the video_gr header file.
 *
 * @param color Integer value of the color to be ignored
 */
void drawBitmapIg(Bitmap* bmp, int x, int y, Alignment alignment, int color);
