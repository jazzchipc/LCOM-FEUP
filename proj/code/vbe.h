...
#define VBE_INT 			0x10		// interrupt vector
#define VBE_CALL_FUNCTION 	0x4f		// AH register should be set to 4F, to distinguish VBA and BIOS functions

// The AL register is used to indicate which VBE function is to be performed
// The BL register is used to indicate a specific subfunction

//VBE return status

#define VBE_SUPP		0x4f	// AL will return 0x4f if function is supported. Any value different means it is not supported

#define VBE_SUCCESS 	0x00	// if the command given was understood
#define VBE_FAIL 		0x01	// if it failed
#define VBE_NOSUPP 		0x02	// if the command is not supported in the current configuration
#define VBE_INVALID 	0x03	// if the function call is invalid in current video mode

// VBE functions

#define VBE_RETURN_VBE_MODE_INFO	0x01	// VBE manual page 16
#define VBE_RETURN_CURRENT_VBE_MODE	0x03	// AH is set to 4F and AL is set to 0x03. So AX will be 0x4F03, as referred in VBE manual (pg. 26)

//VBE graphic modes

#define VBE_GRAPHICS_1024_768_256	0x105
#define VBE_GRAPHICS_1024_768_16B	0x117

//VBE text modes

#define	VBE_TEXT_80_25				0x03
