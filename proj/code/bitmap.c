...
/*
 * Draws a Bitmap on the specified pixels depending on the alignment
 */
void drawBitmap(Bitmap* bmp, int x, int y, Alignment alignment) {
    ...
    for (i = 0; i < height; i++) {
        ...
        bufferStartPos = getSecondaryVideoMem();
...
/*
 * ignores a certain color of a bitmap when it is drawn
 */
void drawBitmapIg(Bitmap* bmp, int x, int y, Alignment alignment, int color) {
    ...
    for (i = 0; i < height; i++) {
        ...
        {
        	unsigned char firstByte = imgStartPos[j];
        	unsigned char secondByte = imgStartPos[j+1];

        	int diff = (firstByte) + (secondByte << 8) - color;

        	//if the color in the array is not a shade of the "color"
